...
- **예약현황 → 출석부 · 문자발송 명단 자동 변환**: 예약 시스템에서 내려받은 `예약현황조회.xlsx`를 올리면 회차(프로그램+이용일)별로 자동 분류하여 지정 양식의 출석부(.xlsx)와 문자발송 프로그램용 명단(CP949 인코딩 CSV)을 만들어 줍니다. **업로드부터 파일 생성까지 전 과정이 브라우저 안에서만 처리되어 신청자 이름·연락처가 서버나 데이터베이스에 저장되지 않습니다.**
...
### 4. 예약현황 → 출석부 변환 파이프라인 (오프라인·무전송)
1. **Input**: 운영자가 `출석부` 탭에서 예약현황조회 엑셀을 올립니다. 여러 프로그램 파일을 한 번에 올릴 수 있습니다.
2. **Parsing**: SheetJS가 브라우저에서 파일을 읽고, 예약완료 + 결제완료 건만 추려 회차별로 묶습니다. (취소 건 포함 여부는 화면에서 전환 가능)
3. **Attendance XLSX**: 인라인된 출석부 양식(`src/lib/attendanceTemplateBase64.ts`)을 ExcelJS로 불러와, 서식이 잡힌 데이터 행을 인원수만큼 복제하고 값만 채웁니다. 양식을 새로 그리지 않습니다.
4. **SMS CSV**: 문자발송 프로그램이 요구하는 CP949(EUC-KR) 바이트로 직접 인코딩합니다. 헤더 없이 `번호,이름` 한 줄씩, 줄바꿈은 CRLF입니다. CP949에 없는 글자는 `?`로 바꾸고 그 건수를 안내합니다.
5. **Download**: 두 결과물 모두 `file-saver`로 즉시 내려받습니다. **네트워크 요청이 단 한 건도 발생하지 않아 오프라인에서도 동작합니다.**

> 출석부 양식을 고쳤다면 `public/sheets/출석부양식.xlsx`를 수정한 뒤 `node scripts/generate-attendance-template.mjs`를 실행해 인라인 base64를 다시 생성해야 합니다.
...
├── public/           # 정적 에셋
│   └── sheets/       # 무손실 엑셀 템플릿(양식.xlsx, 출석부양식.xlsx)
