...
const UPDATE_HISTORY = [
  {
    version: '2.0.0',
    notes: [
      '출석부 탭 신설 — 예약현황조회 파일에서 회차별 출석부(XLSX)와 문자발송 명단(CP949 CSV) 자동 생성',
      '출석부 업로드에 끌어다 놓기 지원 (파일·폴더·ZIP 모두 인식)',
      '예약자 이름과 연락처는 브라우저 안에서만 처리하며 서버나 데이터베이스에 저장하지 않음',
      '첫 로딩 용량 3.6MB → 1.3MB로 축소, 폰트 자체 호스팅으로 오프라인에서도 글꼴 정상 표시',
      '카운터 버튼 확대 및 태블릿 한 화면 배치, 설정·기록 화면 레이아웃 정돈',
      '앱 아이콘·파비콘 신규 제작, 홈 화면 아이콘 정상화',
      '카드 색 번짐·글씨 잘림·대시보드 툴팁 겹침 등 표시 오류 수정, 로그인 팝업 차단 시 현재 창에서 이어서 로그인',
      '새 버전이 배포되면 집계 중에 강제로 새로고침되지 않고, 안내 후 원하는 때 적용',
    ]
  },
  {
    version: '1.9.5',
